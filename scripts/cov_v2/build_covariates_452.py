...
def main():
    ...
    if "basecaller" in md.columns and md["basecaller"].nunique() > 1:
        blevels = sorted(md["basecaller"].dropna().unique())
        for lv in blevels[1:]:
            known[f"basecaller_{lv}"] = (md["basecaller"] == lv).astype(float).values
        print(f"basecaller levels {blevels}; reference={blevels[0]}")
    elif "basecaller" in md.columns:
        print("NOTE: basecaller monomorphic; dropping basecaller")
